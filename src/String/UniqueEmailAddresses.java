...
/*
 * Leetcode Practice: #929. Unique Email Addresses
 * 
...
public class UniqueEmailAddresses {
	...
	public static int numUniqueEmails(String[] emails) {
        ...
        for(int i = 0; i < emails.length; i++) {
            String givenEmail = emails[i];
            String actualEmail = "";
            boolean skip = false, domain = false;
            for(int j = 0; j < givenEmail.length(); j++) {
                char c = givenEmail.charAt(j);
                if(!domain && Character.toString(c).matches("[.]")) {
                    continue;
                }
                ...
                actualEmail += Character.toString(c);
            }
            set.add(actualEmail);
        }
        return set.size();
