package Array;
/*
 * Leetcode Practice: #904. Fruit Into Baskets
 * 
...
public class FruitsIntoBaskets {
	...
	public static int totalFruit(int[] tree) {
        ...
        for(int startingPoint = 0; startingPoint < tree.length - 2; startingPoint++) {
            int total = 0, fruit1 = -1, fruit2 = -1;
            for(int i = startingPoint; i < tree.length; i++) {
                if(fruit1 == -1 || tree[i] == fruit1) {
                    fruit1 = tree[i];
                    total++;
                } else if(fruit2 == -1 || tree[i] == fruit2) {
                    fruit2 = tree[i];
                    total++;
                }
